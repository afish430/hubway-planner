...
function init() {
    ...
    catch (e) { //for IE versions that don't support canvas
        $("#canvasDiv").html("<img src='images/fullHeader.png' width='1000' height='175' />");
        alert("This page may not work properly in IE versions prior to 9. For best results, use Chrome or Firefox.");
    }

    //set up initial dialogs
    $("#startDialog").dialog({ position: { my: "left top", at: "left+80 top+90", of: "#startMap" }, width: 310, height: 175 });
    $("#endDialog").dialog({ position: { my: "left top", at: "left+80 top+90", of: "#endMap" }, width: 310, height: 175});
    $("#startAddress").focus();

    //set initial event handlers
    $("#btnPlanRoute").click(changePageLayout);
    $("#btnReset").click(resetPage);
    $("#btnFindStart").click(findStartAddress);
    $("#btnFindEnd").click(findEndAddress);
    $("#btnUseCurrent").click(getCurrentLocation);
    ...
    mapElement = document.getElementById("endMap");
...
function createCanvas() {
    ...
    // draw image of city skyline
    var cityImage = new Image();
    cityImage.src = "images/skyline2.png";
    cityImage.onload = function () {
        context.drawImage(cityImage, 0, 0, 1000, 175);

        //draw image of biker
        var bikeImage = new Image();
        bikeImage.src = "images/biker.png";
        bikeImage.onload = function () {
            context.drawImage(bikeImage, 435, 105, 100, 75);

            // draw title text
            context.shadowColor = "#0055FF";
            context.shadowOffsetX = 3;
            context.shadowOffsetY = 3;
            context.shadowBlur = 10;
            context.font = "bold 80px arial";
            context.fillStyle = "#FFFFFF";
            context.fillText("HUBWAY    PLANNER", 95, 150);

        } //end bikeImage onload

    }   //end cityImage onload
} //end createCanvas;
...
function changePageLayout() {
    if (selectedStartStation.lat == null || selectedEndStation.lat == null) {  //make sure both stations have been chosen
        $('#errorSpan').html("You must choose both Start and Destination Hubway stations. (Click numbered symbols on map or table rows)<br>");
        return;
    }
    $('#errorMessage').remove(); //remove any error message
    $(".chooseHubDiv").fadeOut();
    hideSmallMaps();
    hideSmallTables();
} //end changePageLayout

function hideSmallMaps(){
    $(".smallMap").slideUp(1000);
    showBigMap();
    $("#btnPlanRoute").hide();
    $("#mapsDiv").css("margin:", "10px");
} //end hideSmallMaps

function hideSmallTables() {
    $(".smallTable").fadeOut(1000);
}
...
function showBigMap() {
    setTimeout(function () {
        $("#mapsDiv").css("height", 560);
        $("#tablesDiv").remove();
        $("#btnReset").css("display", "block");
        $(".bigMap").fadeIn(createRouteMap); //create the biking directions map
        $("#directionsDiv").fadeIn();
        $("#btnPrint").css({ opacity: 0.0, visibility: "visible" }).animate({ opacity: 1.0 }, 1000);
        $('#btnReset').css("clear", "none"); //allow float for reset button
    },
    1000
    );
} //end showBigMap

function activateRouteButton() {
    //active the PLAN ROUTE button only if both dialogs are closed
    if (!$("#startDialog").dialog("isOpen") && !$("#endDialog").dialog("isOpen")) {
        //$("#btnPlanRoute").css({opacity: 0.0, visibility: "visible"}).animate({opacity: 1.0}, 1000);
        $("#btnPlanRoute").attr("disabled", false);
    }
} //end activateRouteButton

function resetPage() { //fades everything out before resetting the page- makes for a smoother transition
    if($(".bigMap").css("display")!="none"){ //if still on start and end maps
        $(".bigMap").fadeOut(500, function () {
            location.reload();
        });
        $("#directionsDiv").fadeOut(500);
        $(".bigButton").fadeOut(500);
    }
    else{ //if final route map is showing
    $(".smallMap").fadeOut(500, function () {
            location.reload();
        });
        $(".tableClass").fadeOut(500);
        $(".dialogClass").fadeOut(500);
        $(".ui-dialog").fadeOut(500);
        $(".chooseHubDiv").fadeOut(500);
        $(".bigButton").fadeOut(500);
    }
} //emd resetPage
...
function findStartAddress() {
    ...
    if (parseInt(numStartStations, 10) != numStartStations) { //make sure an integer
        $('#errorSpan').html("The number of Hubway stations must be an integer between 1 and 10<br>");
        return;
    }
    if (numStartStations < 1 || numStartStations > 10) { //make sure it's in range
        $('#errorSpan').html("The number of Hubway stations must be an integer between 1 and 10<br>");
        return;
    }
    //geocode address
